...
const MAX_X = 1000;                      // This is the max X co-ordinate of the graphics canvas...
const MAX_Y = 800;                      // ... and this is the maximum Y co-ordinate.

let snake,                  // The data structure storing all the segments of the snake
    apples,                 // The data structure storing all the apples that randomly appear
    snakeHead,              // The head of the snake (an X and Y co-ordinate)
    snakeDirection,         // [N]orth, [S]outh, [E]ast, [W]est
    snakeAlive,             // Gets set to false when you lose the game!
    snakeMaxLength,         // How long the snake can grow (increases when you eat apples)
    snakeAlreadyTurned;     // This limits you to one change of direction per animation frame
...
/*-------------------------------------------------------
This section initialises the game state and starts the 
canvas drawing
------------------------------------------------------*/
...
function checkKeyPress (event) {
    ...
    if (event.key == 'r') {      // If you've pressed the r key...
        reset();               // Reset
    }
}
